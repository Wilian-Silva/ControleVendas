...
Public Class FrmRecebimentosEfetuados
    Sub ListarTudo()
        ...
        Try
            ...
            sql = "SELECT * FROM mvto_recebimentos order by id desc"
    ...
    Sub ListarFiltro()
        Try
            ...
            If RbPedido.Checked = True Then
                ...
                dt.DefaultView.RowFilter = "cliente LIKE " & "'%" & TxtPesquisar.Text & "%'"
                DataGrid.DataSource = dt
            End If

            If RbFornecedor.Checked = True Then
                If TxtPesquisar.Text <> "" Then
                    dt.DefaultView.RowFilter = "id_venda = " & TxtPesquisar.Text
                    DataGrid.DataSource = dt
                End If
            End If
    ...
    Private Sub FormatarGrid()
        ...
        DataGrid.Columns(2).HeaderText = "Cod. Cliente"
        DataGrid.Columns(3).HeaderText = "Cliente"
        DataGrid.Columns(4).HeaderText = "Nº Venda"
        DataGrid.Columns(5).HeaderText = "Total Nota"
        DataGrid.Columns(6).HeaderText = "Nº Parcela"
        DataGrid.Columns(7).HeaderText = "Valor Parcela"
        DataGrid.Columns(8).HeaderText = "Valor Pago"
        DataGrid.Columns(9).HeaderText = "Saldo"
        DataGrid.Columns(10).HeaderText = "Juros/Multa"
        DataGrid.Columns(11).HeaderText = "Descontos"
        DataGrid.Columns(12).HeaderText = "Total Pago"
        DataGrid.Columns(13).HeaderText = "Data Emissão"
        DataGrid.Columns(14).HeaderText = "Data Vencimento"
        DataGrid.Columns(15).HeaderText = "Data Pagamento"
    ...
    Private Sub DataGrid_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGrid.CellClick
        If e.RowIndex > -1 Then
            ...
            TxtParcela.Text = DataGrid.CurrentRow.Cells(6).Value
            TxtValorParcela.Text = DataGrid.CurrentRow.Cells(7).Value
            TxtValorPago.Text = DataGrid.CurrentRow.Cells(8).Value
            TxtPortador.Text = DataGrid.CurrentRow.Cells(17).Value
            TxtCliente.Text = DataGrid.CurrentRow.Cells(3).Value
        ...
        If TxtIdReg.Text <> "" And TxtIdRegDup.Text <> "" Then
            ...
            If MsgBox("Deseja excluir registro?", vbYesNo, "Exclusão") = vbYes Then
                ...
                Try
                    ...
                    sql = "DELETE FROM mvto_recebimentos where id = '" & TxtIdReg.Text & "' "
                    ...
                    Dim sql3 As String
                    sql3 = "Select SUM(valor_pago) As valor_pago, valor_parcela FROM mvto_recebimentos WHERE id_duplicata = '" & TxtIdRegDup.Text & "' "
                    ...
                    Dim sql1 As String
                    sql1 = "UPDATE duplicatas_receber SET saldo_duplicata = '" & saldo.Replace(",", ".") & "' WHERE id = '" & TxtIdRegDup.Text & "' "
